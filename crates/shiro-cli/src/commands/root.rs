...
pub fn run() -> Result<CmdOutput, ShiroError> {
    let result = serde_json::json!({
        ...
        "commands": [
            ...
            { "name": "taxonomy", "usage": "shiro taxonomy <add|list|search|browse|relations|relate|assign|import> ...", "subcommands": crate::commands::TAXONOMY_SUBCOMMANDS },
            { "name": "config", "usage": "shiro config <show|get|set> ..." },
            { "name": "doctor", "usage": "shiro doctor [--verify-vector]" },
            { "name": "reindex", "usage": "shiro reindex" },
            { "name": "benchmark", "usage": "shiro benchmark <manifest.json> [--warmup-runs <n>] [--measured-runs <n>]" },
